...
RSpec.describe "POST /auth/signup" do
  ...
  context "with valid params" do
    ...
    it "creates a new user in the database" do
      expect { post "/auth/signup", params: valid_params }.to change(User, :count).by(1)
    end
  end

  context "with a mismatched password confirmation" do
    it "returns 422" do
      post "/auth/signup", params: valid_params.merge(password_confirmation: "different")
  ...
  context "with an invalid username format" do
    it "returns 422 for a username with special characters" do
      post "/auth/signup", params: valid_params.merge(username: "bad user!")
      expect(response).to have_http_status(422)
      expect(json["errors"]).to be_present
    end
  end
  ...
  context "with a missing username" do
    it "returns 422" do
      post "/auth/signup", params: valid_params.merge(username: nil)
      expect(response).to have_http_status(422)
    end
  end
end
